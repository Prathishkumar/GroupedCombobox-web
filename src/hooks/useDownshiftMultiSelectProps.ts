...
export function useDownshiftMultiSelectProps(
    ...
    const rawItems = selector.getOptions();

    // Sort items in grouped order (matching MultiSelectionMenu render order)
    // so downshift's index-to-item mapping is consistent with the visual layout.
    ...
    let items: string[];
    if (hasGroups) {
        // Replicate the same grouping + A-Z sort that MultiSelectionMenu applies
